...
abstract class AbstractController
{
    ...
    public function performRequest(array $postData = [], bool $withHeaders = \false) : ?\stdClass
    {
        try {
            ...
            $response = $client->request($this->getHttpRequestMethod(), $this->constructEndpoint(), [
                ...
                // else composer script will break.
                'headers' => $headers,
            ...
            if ($response->getStatusCode() >= 400) {
                ...
                throw new \RuntimeException(\json_encode($error, \JSON_THROW_ON_ERROR), $response->getStatusCode());
            }
            return $body;
    ...
    {
        if (!isset($settings['account_id'], $settings['consumer_key'], $settings['consumer_secret'], $settings['bearer_token'], $settings['access_token'], $settings['access_token_secret'])) {
